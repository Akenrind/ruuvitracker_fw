...
class GPS:
	...
	def gprmc_received(self, match):
		...
		if self._next_fix.lat != None:
			...
			if self.last_fix.dt:
				item = 'lat:%s, lon:%s, alt:%s, speed:%s, time:%04d-%02d-%02d,%02d:%02d:%02dZZ' %(self.last_fix.lat, self.last_fix.lon, self.last_fix.altitude, self.last_fix.speed, self.last_fix.dt.year, self.last_fix.dt.month, self.last_fix.dt.day, self.last_fix.dt.hh, self.last_fix.dt.mm, self.last_fix.dt.sec)
				print(item)			
