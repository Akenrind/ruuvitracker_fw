...
class GetConfiguration(Service):
    def handle(self):
		...
		if qs and "auth" in str(qs):
			...
			if compare_digest(a, client):
				...
				params = ['track_state', 'track_location', 'measure_interval', 'stream_interval', 'fixed_state_track', 'sleep'] # +time
				resp_str = '{'	 
				for x in params:
					resp_str += "%s:%s," %(x, self.kvdb.conn.get(x)) # Remember to set these in the database
				resp_str += 'time:%s}' % str(time.strftime("%Y-%m-%d", time.gmtime()))
				self.response.payload = resp_str
